...
/**
 * Build FFmpeg arguments for 3G2 conversion
 */
const build3g2FFmpegArgs = (inputPath, outputPath, options = {}) => {
  const {
    quality = 'medium',
    preserveMetadata = true
  } = options;

  const preset = THREE_G2_CONVERSION_PRESETS[quality] || THREE_G2_CONVERSION_PRESETS.medium;
  ...
  // Add H.264 profile and level for CDMA mobile compatibility
  ...
  // AAC audio settings for 3G2
  args.push('-ar', '22050'); // Sample rate (3G2 standard)
...
/**
 * Core 3G2 conversion function
 */
export const convertTo3g2 = async (inputPath, options = {}) => {
  try {
    await ensureOutputDir();

    const inputFilename = path.basename(inputPath);
    const inputExt = path.extname(inputPath);
    const outputFilename = `${uuidv4()}.3g2`;
    const outputPath = path.join(PATHS.outputs, outputFilename);

    logger.info(`Starting 3G2 conversion: ${inputFilename} -> ${outputFilename}`);
    ...
    const args = build3g2FFmpegArgs(inputPath, outputPath, options);

    // Sanitize legacy/unsupported flags (some FFmpeg builds reject `-deinterlace`)
    // If someone accidentally left `-deinterlace` in args, remove it and
    // convert it into the modern yadif filter appended to any existing -vf.
    for (let i = args.length - 1; i >= 0; i--) {
      if (args[i] === '-deinterlace') {
        // remove the legacy flag
        args.splice(i, 1);

        // append yadif to existing -vf or add new -vf entry
        const vfIndex = args.findIndex(a => a === '-vf');
        if (vfIndex !== -1 && typeof args[vfIndex + 1] === 'string') {
          args[vfIndex + 1] = `${args[vfIndex + 1]},yadif=0:-1:0`;
        } else {
          // add new -vf then the filter
          // insert before format flag if possible (look for -f)
          const fIndex = args.findIndex(a => a === '-f');
          if (fIndex !== -1) {
            args.splice(fIndex, 0, '-vf', 'yadif=0:-1:0');
          } else {
            args.push('-vf', 'yadif=0:-1:0');
          }
        }
    ...
    const result = await new Promise((resolve, reject) => {
      ...
      ffmpeg.stderr.on('data', (data) => {
        ...
        if (conversionProgress.frame % 100 === 0 && conversionProgress.frame > 0) {
          logger.info(`3G2 Progress: frame=${conversionProgress.frame}, time=${conversionProgress.time}, speed=${conversionProgress.speed}`);
        }
      });
      ...
      ffmpeg.on('close', (code) => {
        ...
        if (code === 0) {
          logger.info(`3G2 conversion successful: ${outputFilename}`);
          resolve({
            success: true,
            outputPath,
            outputFilename,
          ...
          reject(new Error(`3G2 conversion failed with code ${code}`));
        }
      });
    ...
    logger.error(`3G2 conversion error: ${error.message}`);
    throw error;
  }
};

/**
 * 1. MP4 to 3G2 conversion
 */
export const convertMp4To3g2 = async (inputPath, options = {}) => {
  logger.info('Starting MP4 to 3G2 conversion');
  return await convertTo3g2(inputPath, options);
};

/**
 * 2. WEBM to 3G2 conversion (via MP4 intermediate)
 * WEBM -> MP4 -> 3G2 for better compatibility
 */
export const convertWebmTo3g2 = async (inputPath, options = {}) => {
  let intermediateMp4Path = null;
  
  try {
    await ensureOutputDir();

    logger.info('Starting WEBM to 3G2 conversion (via MP4 intermediate)');
    ...
    // Step 2: Convert MP4 to 3G2
    logger.info('Step 2/2: Converting MP4 to 3G2...');
    const result3g2 = await convertTo3g2(intermediateMp4Path, options);
    ...
    logger.info('WEBM to 3G2 conversion completed successfully');
    
    return {
      ...result3g2,
      inputFilename: path.basename(inputPath),
      conversionMethod: 'webm-mp4-3g2'
    };
    ...
    logger.error(`WEBM to 3G2 conversion error: ${error.message}`);
    throw error;
  }
};

/**
 * 3. AVI to 3G2 conversion
 */
export const convertAviTo3g2 = async (inputPath, options = {}) => {
  logger.info('Starting AVI to 3G2 conversion');
  return await convertTo3g2(inputPath, options);
};

/**
 * 4. MOV to 3G2 conversion
 */
export const convertMovTo3g2 = async (inputPath, options = {}) => {
  logger.info('Starting MOV to 3G2 conversion');
  return await convertTo3g2(inputPath, options);
};

/**
 * 5. MKV to 3G2 conversion
 */
export const convertMkvTo3g2 = async (inputPath, options = {}) => {
  logger.info('Starting MKV to 3G2 conversion');
  return await convertTo3g2(inputPath, options);
};

/**
 * 6. WMV to 3G2 conversion
 */
export const convertWmvTo3g2 = async (inputPath, options = {}) => {
  logger.info('Starting WMV to 3G2 conversion');
  return await convertTo3g2(inputPath, options);
};

/**
 * 7. FLV to 3G2 conversion
 */
export const convertFlvTo3g2 = async (inputPath, options = {}) => {
  logger.info('Starting FLV to 3G2 conversion');
  return await convertTo3g2(inputPath, options);
};

/**
 * 8. MPEG to 3G2 conversion
 */
export const convertMpegTo3g2 = async (inputPath, options = {}) => {
  logger.info('Starting MPEG to 3G2 conversion');
  return await convertTo3g2(inputPath, options);
};

/**
 * 9. 3GP to 3G2 conversion
 */
export const convert3gpTo3g2 = async (inputPath, options = {}) => {
  logger.info('Starting 3GP to 3G2 conversion');
  return await convertTo3g2(inputPath, options);
};
